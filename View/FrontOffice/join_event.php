...
try {
  ...
  // Check if event exists and get max_participent
  $stmt = $pdo->prepare('SELECT id, max_participent FROM events WHERE id = ?');
  ...
  if ($event['max_participent'] !== null && $event['max_participent'] > 0) {
    $stmt = $pdo->prepare('SELECT COUNT(*) as count FROM event_participation WHERE event_id = ?');
    $stmt->execute([$event_id]);
    $countResult = $stmt->fetch();
    $currentCount = (int)$countResult['count'];
    
    if ($currentCount >= $event['max_participent']) {
      $response['message'] = 'This event is full. No more spots available.';
