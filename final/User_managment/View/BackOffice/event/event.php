...
<link rel="stylesheet" href="../../../public/css/stylebackevent.css">
  <style>
    /* ===== ADMIN DASHBOARD ===== */
    body {
      background: #0c0c0c;
 ...
 <body>
  ...
  <div class="sidebar">
    ...
    <a href="../games_dashboard.php">Games</a>
    <a href="../admintour.php">Tournaments</a>
  ...
  <div class="main-content">
    ...
    <section class="events-management">
      ...
      <!-- ===== Search Event by Name Section ===== -->
      <div class="search-event-section" style="margin-bottom: 20px; padding: 15px; background: #0d0d0d; border-radius: 8px; border: 1px solid #1aff8710;">
        <h3 style="color: #1aff87; margin-bottom: 12px;">Search Event by Name</h3>
        <div style="display: flex; gap: 10px; align-items: center;">
          <input 
            type="text" 
            id="searchEventInput" 
            placeholder="Enter event name..." 
            style="flex: 1; padding: 10px; background: #1a1a1a; border: 1px solid #1aff87; color: #fff; border-radius: 6px; font-family: Poppins, Arial, sans-serif;"
            onkeyup="filterEventsByName()"
          >
          <button 
            onclick="clearEventSearch()" 
            style="padding: 10px 20px; background: #ff6464; color: #fff; border: none; border-radius: 6px; cursor: pointer; font-weight: 600;"
          >Clear</button>
        </div>
      </div>

      <!-- ===== Search Event by Period (Month) Section ===== -->
      <div class="search-period-section" style="margin-bottom: 20px; padding: 15px; background: #0d0d0d; border-radius: 8px; border: 1px solid #1aff8710;">
        <h3 style="color: #1aff87; margin-bottom: 12px;">Filter Events by Month/Period</h3>
        <div style="display: flex; gap: 10px; align-items: center; flex-wrap: wrap;">
          <select 
            id="filterMonth" 
            onchange="filterEventsByPeriod()" 
            style="padding: 10px; background: #1a1a1a; border: 1px solid #1aff87; color: #fff; border-radius: 6px; font-family: Poppins, Arial, sans-serif; cursor: pointer;"
          >
      ...
      <!-- ===== Sort Events Section ===== -->
      <div class="sort-events-section" style="margin-bottom: 20px; padding: 15px; background: #0d0d0d; border-radius: 8px; border: 1px solid #1aff8710;">
        <h3 style="color: #1aff87; margin-bottom: 12px;">Sort Events</h3>
        <div style="display: flex; gap: 10px; align-items: center; flex-wrap: wrap;">
          <select 
            id="sortOption" 
            onchange="sortEvents()" 
          ...
          >
            ...
            <option value="participants-most">Participants (Most First)</option>
            <option value="participants-least">Participants (Least First)</option>
      ...
      <div class="events-grid-admin">
        <?php if (!empty($list)) : ?>
            <?php foreach ($list as $event) : ?>
                ...
                ?>
                <div class="event-card-admin" data-id="<?php echo htmlspecialchars($event_id); ?>" data-participant-count="<?php echo $participantCount; ?>">
                    <?php 
                    ...
                    <?php else: ?>
                      <img class="event-asset-visual" src="../../../../public/images/gamelogo.jpg" alt="Event Image">
                    <?php endif; ?>
      ...
      <div class="modal-content">
        ...
        <form id="eventForm" method="POST" action="">
          ...
          <div class="form-group">
            ...
            <input type="number" id="max_participants" name="max_participants" min="1" value="<?php echo htmlspecialchars($editEvent['max_participants'] ?? ''); ?>" placeholder="Leave empty for unlimited">
            <div id="error-max_participants" style="color:#ff6464; font-size:0.9rem; margin-top:6px;"></div>
    ...
    <script src="../../../public/js/eventback.js"></script>
    <script>
      // Search event by name functionality
      function filterEventsByName() {
        const searchInput = document.getElementById('searchEventInput');
        const searchTerm = searchInput ? searchInput.value.toLowerCase().trim() : '';
        const eventCards = document.querySelectorAll('.event-card-admin');
        
        eventCards.forEach(card => {
          const eventTitle = card.querySelector('h4');
          if (eventTitle) {
      ...
      // Optional: Allow Enter key to trigger search (already works with onkeyup, but explicit for clarity)
      ...
      function filterEventsByPeriod() {
        ...
        const selectedYear = document.getElementById('filterYear') ? document.getElementById('filterYear').value : '';
        const eventCards = document.querySelectorAll('.event-card-admin');
        
        eventCards.forEach(card => {
          const eventDateEl = card.querySelector('.event-date');
          if (eventDateEl) {
            const eventDateText = eventDateEl.textContent.replace('📅 ', '').trim();
            let isMatch = true;
            
            // Check if month matches (if month filter is applied)
            if (selectedMonth !== '') {
              // Extract month from date string (handles formats like "2025-01-15", "January 15, 2025", etc.)
              const monthMatch = eventDateText.match(/(\d{4})-(\d{2})/); // YYYY-MM format
              if (monthMatch) {
      ...
      function sortEvents() {
        const sortOption = document.getElementById('sortOption') ? document.getElementById('sortOption').value : '';
        const eventsGrid = document.querySelector('.events-grid-admin');
        if (!eventsGrid) return;
        
        const eventCards = Array.from(document.querySelectorAll('.event-card-admin'));
        ...
        eventCards.sort((a, b) => {
          switch(sortOption) {
            ...
            case 'date-newest': {
              const dateA = a.querySelector('.event-date')?.textContent.replace('📅 ', '').trim() || '';
              const dateB = b.querySelector('.event-date')?.textContent.replace('📅 ', '').trim() || '';
            ...
            case 'participants-most': {
              const countA = parseInt(a.getAttribute('data-participant-count') || '0');
              const countB = parseInt(b.getAttribute('data-participant-count') || '0');
