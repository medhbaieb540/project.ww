...
$deletedGames = $gameModel->getDeletedGames();
$deletedCount = count($deletedGames);
...
<head>
  ...
  <link rel="stylesheet" href="../../public/css/styleGames.css">
  <style>
    /* ===== ADMIN DASHBOARD ===== */
    body {
      background: #0c0c0c;
...
<body>
  <!-- ===== Sidebar ===== -->
   <div class="sidebar">
    ...
    <a href="games_dashboard.php" class="nav-link active">Games</a>
  ...
  <div class="main-content">
    ...
    <div style="display: flex; gap: 12px; flex-wrap: wrap; align-items: center;">
      ...
      <button onclick="toggleTrash()" class="btn-add" style="background:#ff3366;color:#fff;">🗑️ Trash (<?= $deletedCount ?>)</button>
    </div>
    ...
    <!-- ===== Trash (Deleted Games) ===== -->
    <div id="trash-section" class="table-container" style="display:none; border-color:#ff3366; margin-top:20px;">
      <div style="display:flex; justify-content: space-between; align-items:center; margin-bottom:10px;">
        <h3 style="color:#ff3366; margin:0;">🗑️ Deleted Games (<?= $deletedCount ?>)</h3>
        <?php if ($deletedCount > 0): ?>
          <a href="<?= $baseUrl ?>/index.php?controller=game&action=emptyTrash"
             class="btn-action btn-delete"
             onclick="return confirm('Delete all items in trash permanently?');">Empty Trash</a>
        <?php endif; ?>
      </div>
      <?php if (empty($deletedGames)): ?>
        <div class="empty-state">No deleted games yet.</div>
      <?php else: ?>
        <table>
          <thead>
            <tr>
          ...
          <tbody>
            <?php foreach ($deletedGames as $game): ?>
              <tr>
                ...
                <td><?= htmlspecialchars($game['category_name'] ?? 'Uncategorized'); ?></td>
                <td><?= htmlspecialchars($game['developer_name'] ?? 'Unknown'); ?></td>
                <td><?= date('Y-m-d H:i', strtotime($game['deleted_at'])); ?></td>
                <td>
                  <a href="<?= $baseUrl ?>/index.php?controller=game&action=restore&id=<?= $game['game_id']; ?>"
                     class="btn-action btn-edit">Restore</a>
                  <a href="<?= $baseUrl ?>/index.php?controller=game&action=permanentDelete&id=<?= $game['game_id']; ?>"
                     class="btn-action btn-delete"
                     onclick="return confirm('Permanently delete this game?');">Delete Forever</a>
                </td>
              </tr>
            <?php endforeach; ?>
          </tbody>
    ...
    <!-- ===== Trash (Deleted Games) ===== -->
    <h2 class="section-title" id="trash-section">| Trash (Deleted Games)</h2>
    <div class="table-container">
      <?php if (empty($deletedGames)): ?>
        <div class="empty-state">No deleted games yet.</div>
      <?php else: ?>
          ...
          <tbody>
            <?php foreach ($deletedGames as $game): ?>
              <tr>
                ...
                <td><?php echo htmlspecialchars($game['category_name'] ?? 'Uncategorized'); ?></td>
                <td><?php echo htmlspecialchars($game['developer_name'] ?? 'Unknown'); ?></td>
                <td><?php echo date('Y-m-d H:i', strtotime($game['deleted_at'])); ?></td>
                <td>
                  <a href="<?= $baseUrl ?>/index.php?controller=game&action=restore&id=<?php echo $game['game_id']; ?>" class="btn-action btn-edit" onclick="return confirm('Restore this game?')">Restore</a>
                  <a href="<?= $baseUrl ?>/index.php?controller=game&action=permanentDelete&id=<?php echo $game['game_id']; ?>" class="btn-action btn-delete" onclick="return confirm('Permanently delete this game? This cannot be undone.')">Delete Forever</a>
                </td>
              </tr>
            <?php endforeach; ?>
          </tbody>
