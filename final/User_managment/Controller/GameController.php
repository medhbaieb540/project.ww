...
class GameController {
    ...
    public function index() {
        ...
        if (!isset($deletedGames)) $deletedGames = [];
        if (!isset($deletedCount)) $deletedCount = 0;
        
        require_once __DIR__ . '/../View/FrontOffice/list.php';
    ...
    public function create() {
        $categories = $this->categoryModel->getAllCategories();
        require_once __DIR__ . '/../View/BackOffice/game/create.php';
    }
    ...
    public function store() {
        if ($_SERVER['REQUEST_METHOD'] === 'POST') {
            error_log("=== Starting game upload ===");
            ...
            error_log("Game data: " . print_r($data, true));
            ...
            if (!empty($errors)) {
                ...
                error_log('Game validation errors: ' . implode(', ', $errors));
                header('Location: ../../View/FrontOffice/list.php?controller=game&action=create');
                exit;
            }
            ...
            if (isset($_FILES['game_file']) && $_FILES['game_file']['error'] === UPLOAD_ERR_OK) {
                ...
                if ($filePath) {
                    ...
                    error_log('Game file upload failed');
                    header('Location: ../../View/FrontOffice/list.php?controller=game&action=create');
                    exit;
                }
            }
            ...
            error_log("Create game result: " . ($result !== false ? "SUCCESS (ID: $result)" : "FAILED"));
            
            if ($result !== false) {
                $_SESSION['success'] = 'Game created successfully!';
                error_log("Game created with ID: $result - Redirecting to list");
                header('Location: ../../View/FrontOffice/list.php?controller=game&action=index');
                exit;
            } else {
                $_SESSION['error'] = 'Failed to create game. Please try again.';
                error_log('Game creation failed: ' . print_r($data, true));
                header('Location: ../../View/FrontOffice/list.php?controller=game&action=create');
                exit;
            }
        }
    ...
    public function update() {
        if ($_SERVER['REQUEST_METHOD'] === 'POST') {
            ...
            if ($result) {
                ...
                header('Location: ../../View/FrontOffice/list.php?controller=game&action=edit&id=' . $id);
                exit;
            }
        }
    }
